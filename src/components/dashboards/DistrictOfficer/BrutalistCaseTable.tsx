...
interface BrutalistCaseTableProps {
  onOpenDisbursementModal: (caseItem: (typeof MOCK_DISTRICT_CASES)[0]) => void;
...
export const BrutalistCaseTable: React.FC<BrutalistCaseTableProps> = ({
  ...
  const filteredCases = MOCK_DISTRICT_CASES.filter((c) => {
    const matchesSearch =
      c.surveyNo.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="gov-card p-6 sm:p-7 mb-8">
      {/* Table Title & 44px Query Controls Toolbar */}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search Survey #, Owner, Village, UID..."
              className="gov-input pl-10 w-60 sm:w-72"
            />
          </div>
      ...
      <div className="pt-4 mt-4 flex flex-wrap items-center justify-between text-xs text-slate-600 font-mono gap-3 border-t border-slate-100">
        <div className="flex items-center gap-4">
          <span>Showing <strong>1–{filteredCases.length}</strong> of <strong>1,842</strong> Cadastral Parcels</span>
          <span className="hidden sm:inline text-slate-300">|</span>
          <span className="hidden sm:inline text-emerald-700 font-bold">PFMS Gateway Disbursal Batch: #2026-08-MH-9812</span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="px-1 text-slate-400">/ 307</span>
          <button
